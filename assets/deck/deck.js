...
function boot(){
  ...
  const SPAN       = 3.0;    // путь карты по вертикали до возврата наверх
  ...
  const STACK_EDGE     = 0.05;   // доля прокрутки страницы на «взрыв»/«стяжку» у края
  ...
  const STACK_BOT_AT   = 0.40;   // НИЖНЯЯ пачка, прячется за фото контактов.
  ...
  const STACK_LAG      = 0.65;   // насколько горизонталь отстаёт от вертикали при
                                 //   вылете из пачки (и опережает её при влёте)
  const STACK_BURST    = 0.15;   // доп. толчок вниз в момент вылета/влёта,
